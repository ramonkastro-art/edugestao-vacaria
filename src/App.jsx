...
function ServidorModal({ servidor, onClose, canTransfer }) {
  ...
  const cadastro = servidor.cadastro ?? null;
  ...
  const escolasVisiveis =
    escolasNomeacao.length > 0 ? escolasNomeacao : escolas;
      ...
      >
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                <span
                  className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${servidor.status === "Ativo" ? "bg-emerald-500/20 text-emerald-300" : "bg-amber-500/20 text-amber-300"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${servidor.status === "Ativo" ? "bg-emerald-400" : "bg-amber-400"}`}
                  />
                  {servidor.status}
                </span>
                {nomeacoes.length > 1 && (
            ...
            {nomeacoes.length > 0
              ? nomeacoes.map((n, i) => (
                  ...
                  >
                    ...
                    {n.escola?.tipo && (
                      <Badge className={TIPO_COLORS[n.escola.tipo]}>
                        {n.escola.tipo}
                      </Badge>
                    )}
                  </div>
              ...
              <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-700">
                    {new Date(
                      cadastro.data_nascimento + "T12:00:00",
          ...
          <div className="px-5 pb-4">
            <div className="p-4 bg-slate-50 rounded-2xl text-center">
              ...
              <p className="text-xs text-slate-300 mt-1">
                Nome não localizado na base cadastral
              </p>
            </div>
          </div>
        )}
        ...
        <div className="px-5 pb-5 pt-2 flex gap-2">
          {canTransfer && (
            <button
              onClick={() =>
                alert("Módulo de transferência em desenvolvimento")
              }
...
function SearchOverlay({ onClose, onOpenServidor, onSelectSchool }) {
  ...
  const total =
    (results.servidores?.length ?? 0) + (results.escolas?.length ?? 0);

  return (
    <div
      className="fixed inset-0 z-50 flex items-end md:items-start md:justify-center md:pt-20 bg-black/30 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
        className="bg-white w-full md:max-w-xl md:mx-4 rounded-t-3xl md:rounded-2xl shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-center pt-3 pb-1 md:hidden">
          <div className="w-10 h-1 rounded-full bg-slate-200" />
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-100">
          ...
          {query ? (
            ...
            <button
              onClick={onClose}
              className="text-xs text-slate-400 px-2 py-1 hover:bg-slate-100 rounded-lg"
            >
              Fechar
            </button>
          )}
        ...
        <div className="max-h-[60vh] overflow-y-auto">
          {query.length >= 2 && !searching && total === 0 && (
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-xs text-slate-300 mt-1">
                Tente partes do nome: "Ana Velho"
              </p>
            </div>
          )}
          {query.length < 2 && (
            <div className="px-4 py-8 text-center">
              ...
              <p className="text-xs text-slate-300 mt-1">
                "Ana Velho" encontra "Ana Tshiedel Velho"
              </p>
            </div>
          )}
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={label} className={`${bg} rounded-2xl p-4`}>
            <div
              className={`w-8 h-8 rounded-xl ${ib} flex items-center justify-center mb-2`}
            >
              <Icon size={15} />
            </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {["EMEF", "EMEI", "EMEF Campo"].map((tipo) => {
            ...
            return (
              <div
                key={tipo}
                className="p-3 bg-white border border-slate-100 rounded-2xl"
              >
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          {escolas.length} escolas · Rede Municipal
        </p>
      </div>
      <div className="space-y-2">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {loading ? "…" : `${professores.length} servidores`} ·{" "}
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">
            {servidores.length} servidores · rede municipal
          </p>
        </div>
        <button
          onClick={reload}
        ...
        {filtered.map((s) => {
          const escAtual =
            s.nomeacoes?.length > 0
              ? [
                  ...new Set(
                    s.nomeacoes.map((n) => n.escola?.name).filter(Boolean),
                  ),
                ].join(" · ")
              : (s.cadastro?.escola_raw?.split(",")[0].trim() ?? "—");
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`w-1.5 h-1.5 rounded-full shrink-0 ${s.status === "Ativo" ? "bg-emerald-500" : "bg-amber-400"}`}
                  />
                </div>
...
function EfeModule({ onOpenServidor }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {escolaFiltro && (
        <p className="text-xs text-slate-400">
          {filtered.length} servidores {saving && "· salvando…"}
        </p>
      )}
      <div className="space-y-2">
        {filtered.map((s) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                <button
                  onClick={() => escolaSel && salvarEfe(s.id, "ok", null)}
                  disabled={!escolaSel}
                ...
                <select
                  ...
                  onChange={(e) =>
                    escolaSel && salvarEfe(s.id, "ocorrencia", e.target.value)
                  }
...
export default function App() {
  ...
  return (
    ...
    >
      {/* Sidebar desktop */}
      <aside
        className={`${sideW} hidden md:flex flex-col bg-white border-r border-slate-100 fixed top-0 left-0 h-screen z-30 transition-all duration-200`}
      >
      ...
      {/* Main */}
      <div
        className={`flex flex-col min-h-screen ${mainML} md:transition-all md:duration-200`}
      >
        <header className="h-14 bg-white border-b border-slate-100 flex items-center gap-2 px-3 sm:px-4 shrink-0 sticky top-0 z-20">
          ...
          >
            ...
            <span className="flex-1 text-left truncate text-xs sm:text-sm">
              Buscar servidor ou escola...
            </span>
            <kbd className="hidden sm:inline text-xs bg-white border border-slate-200 px-1.5 py-0.5 rounded-md font-mono shrink-0">
              ⌘K
            </kbd>
          </button>
        ...
        <main className="flex-1 p-4 sm:p-6 pb-24 md:pb-6 max-w-5xl w-full">
          ...
          {view === "servidores" && (
            <ServidoresList onOpenServidor={setSelectedServidor} />
          )}
